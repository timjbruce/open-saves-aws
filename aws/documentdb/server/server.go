...
func NewServer(config *Config) (*Server, error) {
	log.Printf("DEBUG: Creating DocumentDB store with connection: %s", config.AWS.DocumentDB.ConnectionString)
	
	// Create DocumentDB store
	store, err := NewDocumentDBStore(
		config.AWS.DocumentDB.ConnectionString,
		config.AWS.DocumentDB.PasswordSecretArn,
		config.AWS.DocumentDB.DatabaseName,
	)
	if err != nil {
		log.Printf("ERROR: Failed to create DocumentDB store: %v", err)
		return nil, fmt.Errorf("failed to create DocumentDB store: %v", err)
	}
	
	log.Printf("DEBUG: DocumentDB store created successfully")
