...
data "aws_ssm_parameter" "documentdb_password_secret_arn" {
...
# IAM Policies for Service Account - DocumentDB (via Secrets Manager)
resource "aws_iam_role_policy" "documentdb_policy" {
  name = "open-saves-documentdb-policy-${var.architecture}"
  ...
  policy = jsonencode({
    ...
    Statement = [
      {
        # Secrets Manager access for DocumentDB credentials
        Action = [
          "secretsmanager:GetSecretValue"
        ]
        Effect   = "Allow"
        Resource = data.aws_ssm_parameter.documentdb_password_secret_arn.value
      }
    ]
  })
}
    ...
    template {
      ...
      spec {
        ...
        container {
          ...
          args    = ["--config", "/open-saves/dev/config"]
...
# Create consolidated configuration parameter for the application
resource "aws_ssm_parameter" "app_config" {
  name        = "/open-saves/dev/config"
  description = "Open Saves DocumentDB application configuration"
  type        = "String"
  overwrite   = true
  value = jsonencode({
    server = {
    ...
    aws = {
      region = var.region
      documentdb = {
        connection_string    = data.aws_ssm_parameter.documentdb_username.value != "" ? "mongodb://${data.aws_ssm_parameter.documentdb_username.value}@${data.aws_ssm_parameter.documentdb_endpoint.value}:${data.aws_ssm_parameter.documentdb_port.value}/?ssl=true&replicaSet=rs0&readPreference=secondaryPreferred&retryWrites=false" : ""
        password_secret_arn  = data.aws_ssm_parameter.documentdb_password_secret_arn.value
      ...
      elasticache = {
        address = "${data.aws_ssm_parameter.redis_endpoint.value}:6379"
        ttl     = 3600
      }
      ecr = {
